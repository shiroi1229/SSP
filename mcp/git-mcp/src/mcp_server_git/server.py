...
class GitPull(BaseModel):
    ...
    remote: str = Field(
        "origin",
        description="Remote name to pull from (defaults to 'origin').",
    )
    branch: Optional[str] = Field(
        None,
        description="Branch to pull. Defaults to the current branch if omitted.",
    )
...
class GitPush(BaseModel):
    ...
    branch: Optional[str] = Field(
        None,
        description="Branch to push. Defaults to the current branch if omitted.",
    )
...
def git_fetch(repo: git.Repo, remote: str = "origin") -> str:
    ...
    return f"Fetch from {remote} completed:\n{result}" if result else f"Fetch from {remote} completed"


def git_pull(repo: git.Repo, remote: str = "origin", branch: str | None = None) -> str:
    target_branch = branch or repo.active_branch.name
    result = repo.git.pull(remote, target_branch)
    return f"Pull of {target_branch} from {remote} completed:\n{result}" if result else f"Pull of {target_branch} from {remote} completed"


def git_push(repo: git.Repo, remote: str = "origin", branch: str | None = None) -> str:
    target_branch = branch or repo.active_branch.name
    result = repo.git.push(remote, target_branch)
    return f"Push of {target_branch} to {remote} completed:\n{result}" if result else f"Push of {target_branch} to {remote} completed"
...
async def serve(repository: Path | None) -> None:
    ...
    async def list_tools() -> list[Tool]:
        return [
            ...
            Tool(
                name=GitTools.FETCH,
                description="Fetches updates from a remote",
                inputSchema=GitFetch.model_json_schema(),
            ),
            Tool(
                name=GitTools.PULL,
                description="Pulls the latest changes from a remote branch",
                inputSchema=GitPull.model_json_schema(),
            ),
            Tool(
                name=GitTools.PUSH,
                description="Pushes local commits to a remote branch",
