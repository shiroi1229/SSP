#!/usr/bin/env python3
"""Fail when tracked files contain credential patterns or unsafe generated artifacts."""
...
def is_forbidden_generated_artifact(path: Path) -> bool:
...
def main() -> int:
    ...
    for path in tracked_files():
        rel = path.relative_to(ROOT)
        if is_forbidden_generated_artifact(path):
            findings.append(f"{rel}: generated Python bytecode is tracked")
            continue
    ...
    print("Secret scan passed: no tracked credential patterns or forbidden generated artifacts detected.")
    return 0
